  ...
  "relative w-full rounded-lg border px-4 py-3 text-sm grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current",
  {
    variants: {
      variant: {
        ...
        info:
          "border-blue-200 bg-blue-50 text-blue-900 [&>svg]:text-blue-600 *:data-[slot=alert-description]:text-blue-800",
        success:
...
function AlertTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "col-start-2 line-clamp-1 min-h-4 font-semibold tracking-tight text-sm leading-5",
        className,
      )}
      {...props}
    />
...
function AlertDescription({
  ...
  return (
    <div
      ...
      className={cn(
        "col-start-2 grid justify-items-start gap-1 text-sm leading-relaxed mt-1 opacity-90",
        className,
      )}
      {...props}
    />
