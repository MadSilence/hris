...
import { DesactLogo } from './DesactLogo';
...
export const MainSidebar: React.FC<MainSidebarProps> = ({
  ...
  const componentsByCategory = components.reduce((acc, component) => {
    if (component && component.category && component.name) {
  ...
  const filteredComponentsByCategory = Object.entries(componentsByCategory).reduce((acc, [category, categoryComponents]) => {
    const filtered = categoryComponents.filter(component =>
      component && component.name && component.name.toLowerCase().includes(sidebarSearchTerm.toLowerCase())
    );
