...
const SettingsCard = ({
  ...
  return (
    <Card
      className="h-full p-6 grid gap-2 content-start"
      role="region"
    ...
    >
      <div className="flex items-center gap-4">
        {icon ? (
          <span
            className="inline-grid place-items-center w-8 h-8 rounded-md bg-muted text-muted-foreground"
            aria-hidden
          >
            {icon}
          </span>
        ...
        {items.map((it, idx) => {
          const row = (
            ...
            >
              ...
              {it.external ? (
                <ExternalLink
                  className="w-3.5 h-3.5 text-muted-foreground opacity-70 group-hover:opacity-100"
                  aria-hidden
                />
              ) : null}
            </div>
            ...
            <div key={it.href} className="grid">
              {it.external ? (
                <a
                  ...
                  className="rounded-md no-underline hover:no-underline"
                >
                  {row}
                </a>
              ) : (
