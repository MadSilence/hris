...
export interface RolesPageViewProps {
  ...
  /** Every role, unfiltered — for the assign form and the name-collision check. */
  allRoles?: Role[];
      ...
      <CardHeader className="px-0 py-0 shrink-0">
        <div className="space-y-3">
          ...
          <ListToolbar
            ...
            archived={
              view === "roles" && onShowArchivedChange
                ? {
                    count: archivedRoleCount,
            ...
            primary={
              <RolesPageHeader
                existingRoleNames={(allRoles ?? roleRows ?? []).map((role) => role.name)}
