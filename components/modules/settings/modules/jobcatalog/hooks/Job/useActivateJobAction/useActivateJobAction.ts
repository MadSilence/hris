...
import type { ActivateJobActionInput } from "@/components/modules/settings/modules/jobcatalog/actions/Job/activateJobAction";
import { activateJobAction } from "@/components/modules/settings/modules/jobcatalog/actions/Job/activateJobAction";
 ...
 * invalidate.
 */
export const useActivateJobAction = () => {
  const invalidateJobFamilyQuery = useInvalidateJobFamilyQuery();

  return useMutation({
    mutationFn: (payload: ActivateJobActionInput) => activateJobAction(payload),
    onSuccess: () => {
