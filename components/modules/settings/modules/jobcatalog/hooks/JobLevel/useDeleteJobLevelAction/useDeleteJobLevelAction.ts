...
import type { DeleteJobLevelActionInput } from "@/components/modules/settings/modules/jobcatalog/actions/JobLevel/deleteJobLevelAction";
import { deleteJobLevelAction } from "@/components/modules/settings/modules/jobcatalog/actions/JobLevel/deleteJobLevelAction";
 ...
 */
export const useDeleteJobLevelAction = () => {
  const invalidateGroups = useInvalidateJobLevelGroupsQuery();
  const invalidateFamilies = useInvalidateJobFamilyQuery();

  return useMutation({
    mutationFn: (payload: DeleteJobLevelActionInput) => deleteJobLevelAction(payload),
    onSuccess: () => {
      invalidateGroups();
      invalidateFamilies();
    },
