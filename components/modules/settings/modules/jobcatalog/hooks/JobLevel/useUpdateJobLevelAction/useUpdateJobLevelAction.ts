...
import type { UpdateJobLevelActionInput } from "@/components/modules/settings/modules/jobcatalog/actions/JobLevel/updateJobLevelAction";
import { updateJobLevelAction } from "@/components/modules/settings/modules/jobcatalog/actions/JobLevel/updateJobLevelAction";
 ...
 */
export const useUpdateJobLevelAction = () => {
  const invalidateGroups = useInvalidateJobLevelGroupsQuery();
  const invalidateFamilies = useInvalidateJobFamilyQuery();

  return useMutation({
    mutationFn: (payload: UpdateJobLevelActionInput) => updateJobLevelAction(payload),
    onSuccess: () => {
      invalidateGroups();
      invalidateFamilies();
    },
