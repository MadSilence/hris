...
type Props = {
  ...
  renderPersonChip?: (person: DepartmentPerson) => React.ReactNode;
};

/**
 * People with no department at all. Doubles as the drop target for taking someone out of the
 * structure, and as a standing report of who was never placed.
 */
export function DepartmentUnassignedTab({
  people,
  isLoading,
  selectedPersonId,
  onSelectPerson,
  ...
  if (people.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3 px-6 text-center">
        ...
        <div>
          <p className="text-sm font-medium text-brown-900">Everyone has a department</p>
    ...
    <div className="flex h-full min-h-0 flex-col gap-3 p-6">
      <div className="flex-none">
        ...
        <p className="mb-3 text-sm text-brown-500">
          {people.length} {people.length === 1 ? "person is" : "people are"} not in any department.
        </p>
