...
import { useDepartmentMembers } from "@/components/modules/settings/modules/departments/hooks/useDepartmentMembers/useDepartmentMembers";
import { useRemoveDepartmentMember } from "@/components/modules/settings/modules/departments/hooks/useRemoveDepartmentMember/useRemoveDepartmentMember";
import { PermissionGate } from "@/components/auth/PermissionGate";
import { Button } from "@/public/desact/src/components/ui/button";
import type { DepartmentMember } from "@/models/departments";
...
export function DepartmentMembersList({ departmentId, isArchived }: Props) {
  const [page, setPage] = useState(0);
  const [removingId, setRemovingId] = useState<string | null>(null);
  const { data, isLoading, error } = useDepartmentMembers(departmentId, page, PAGE_SIZE);
  const removeMember = useRemoveDepartmentMember();

  const handleRemove = async (userId: string) => {
    setRemovingId(userId);
    try {
      await removeMember.mutateAsync({ id: departmentId, userId });
    } finally {
      setRemovingId(null);
    }
  };
  ...
  if (items.length === 0) {
    return (
      <p className="text-sm text-brown-400">No members assigned yet.</p>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-1">
      {items.map((member) => (
        <div
          key={member.userId}
          className="flex items-center gap-3 py-2 px-1 rounded-md hover:bg-brown-50 group"
        >
