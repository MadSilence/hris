...
export type DepartmentSearchResult = {
  /** Ids of the departments matching the query, in canvas (depth-first) order. */
  matchIds: string[];
  matchSet: Set<string>;
  /** Ancestor chain of every department, so a picked result can be revealed on demand. */
...
export function useDepartmentSearch(
  ...
  return useMemo(() => {
    ...
    const walk = (node: DepartmentTreeNode, ancestors: string[]) => {
