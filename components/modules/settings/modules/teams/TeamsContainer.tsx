...
import { CanvasLoading, CanvasMessage } from "@/components/ui/Canvas";
...
/**
 * Search is teams-only for now, so the units/people switch is not drawn. The people
...
export default function TeamsContainer() {
  ...
  /**
   * What the toolbar Add button pre-fills as the parent. An archived team is not offered as a
   * parent in the dialog, so selecting one falls back to the root.
   */
  const defaultNewParentId =
    selectedTeam && selectedTeam.status === "ACTIVE" ? selectedTeam.id : null;
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <ExportOrgTreeModal
        ...
        noun="team"
        nodes={exportNodes}
        defaultNodeId={selectedTeam?.id ?? null}
        buildExportUrl={(id) => `/api/teams/${id}/export`}
      />
