...
import type { TeamNode } from "../TeamTree/TeamTree";
...
export function TeamDetailsPanel({ team }: Props) {
  const [activeTab, setActiveTab] = useState("overview");

  const directReports = team.directReports ?? team.children?.length ?? 0;
    ...
    <div className="flex flex-col h-full min-h-0 p-6 gap-5">
      ...
      <div className="flex items-center gap-4 flex-none">
        ...
        <div className="min-w-0">
          ...
          {team.about && (
            <p className="text-sm text-brown-500 truncate">{team.about}</p>
          )}
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          {/* Responsibilities */}
          {(team.responsibilities?.length ?? 0) > 0 && (
            <div>
              <h3 className="text-xs font-semibold uppercase tracking-wide text-brown-400 mb-2">
                Responsibilities
              </h3>
