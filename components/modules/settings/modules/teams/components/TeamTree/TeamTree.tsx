...
type Props = {
  ...
  onAdd: () => void;
  onEdit: (node: TeamTreeNode) => void;
  onArchive: (node: TeamTreeNode) => void;
  onActivate: (node: TeamTreeNode) => void;
  onDelete: (node: TeamTreeNode) => void;
};

function collectMatchingAndAncestors(nodes: TeamTreeNode[], query: string): Set<string> {
  const result = new Set<string>();
  function traverse(node: TeamTreeNode, ancestors: string[]): boolean {
  ...
  return (
    ...
    >
      <path
        ...
        fill="currentColor" fillOpacity={open ? 0.18 : 0.08}
      />
      <path
        d="M1.5 4.5A1 1 0 0 1 2.5 3.5h3.086l1 1H13.5a1 1 0 0 1 1 1v7a1 1 0 0 1-1 1h-11a1 1 0 0 1-1-1V4.5z"
...
export function TeamTree({
  data, selectedId, defaultExpandedIds = [], onSelect, onAdd, onEdit, onArchive, onActivate, onDelete,
}: Props) {
  ...
  function renderNode(node: TeamTreeNode, depth: number): React.ReactNode {
      ...
      <div key={node.id}>
        ...
        >
          ...
          <span className="flex-1 truncate leading-snug">
            {node.name}
            {isArchived && <span className="ml-1.5 text-xs text-brown-400 font-normal">(archived)</span>}
          </span>

          <Badge variant="secondary" className="ml-1 flex-none h-5 min-w-[20px] px-1.5 text-xs font-normal rounded-md">
            {node.memberCount}
          </Badge>

          <PermissionGate anyOf={[{ resource: "ORG.TEAM", action: "EDIT" }, { resource: "ORG.TEAM", action: "MANAGE" }]}>
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" onClick={(e) => e.stopPropagation()}>
                ...
                <PermissionGate resource="ORG.TEAM" action="MANAGE">
                  <DropdownMenuItem onSelect={() => onDelete(node)} className="text-red-600 focus:text-red-600">
                    Delete
                  </DropdownMenuItem>
                </PermissionGate>
              </DropdownMenuContent>
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      <div className="p-3 border-b border-brown-200 flex-none">
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            <svg className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-brown-400 pointer-events-none"
              viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <circle cx="11" cy="11" r="8" /><path d="m21 21-4.35-4.35" />
            </svg>
            <Input placeholder="Search teams..." value={query} onChange={(e) => setQuery(e.currentTarget.value)} className="pl-8 h-9 text-sm" />
          </div>
          <PermissionGate resource="ORG.TEAM" action="EDIT">
            <Button size="icon" aria-label="Add team" onClick={onAdd}
      ...
      <div className="flex-1 overflow-y-auto p-2 min-h-0">
        {data.length === 0 ? (
          <div className="flex items-center justify-center h-20 text-sm text-brown-400">No teams found.</div>
        ) : (
