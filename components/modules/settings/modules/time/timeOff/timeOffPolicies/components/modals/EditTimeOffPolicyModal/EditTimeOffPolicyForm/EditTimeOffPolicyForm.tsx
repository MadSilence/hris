...
export type EditTimeOffPolicyFormValues = {
  name: string;
  description: string;
...
type Props = {
  ...
  onSubmitAction: (values: EditTimeOffPolicyFormValues) => void | Promise<void>;
};
...
export const EditTimeOffPolicyForm: FC<Props> = ({
  policy,
  isLoading = false,
  onCancelAction,
  onDirtyChangeAction,
  ...
  const handleFormSubmission = useCallback(
    (values: EditTimeOffPolicyFormValues) =>
      onSubmitAction({ ...values, name: values.name.trim(), description: values.description.trim() }),
    [onSubmitAction],
  );

  const formik = useFormik<EditTimeOffPolicyFormValues>({
    initialValues: {
      ...
      <div className="max-h-[65vh] overflow-y-auto px-6 py-5">
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="edit-policy-unit">Unit</Label>
              <Select
                ...
                disabled={isLoading}
              >
                <SelectTrigger id="edit-policy-unit" aria-invalid={!!formik.errors.unit}>
            ...
            <div className="space-y-2">
              <Label htmlFor="edit-policy-paid">Pay type</Label>
              <Select
                ...
                disabled={isLoading}
              >
                <SelectTrigger id="edit-policy-paid">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
